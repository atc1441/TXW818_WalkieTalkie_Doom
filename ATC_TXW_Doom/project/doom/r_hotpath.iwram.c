...
#include "config.h"
    ...
    #define DOOM_PC_BUILD
    #include <time.h>
#else
    #include <stdint.h>
    extern uint64_t os_jiffies(void);
    extern uint64_t os_jiffies_to_msecs(uint64_t jiff);
#endif
...
#else
    thistimereply = (int)(os_jiffies_to_msecs(os_jiffies()) * TICRATE / 1000);
#endif
