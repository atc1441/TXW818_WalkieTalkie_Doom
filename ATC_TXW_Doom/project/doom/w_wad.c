...
//
// VIRTUAL LUMP TABLE
// Lumps not present in the WAD can be registered here at runtime.
// They receive lump numbers >= VIRTUAL_LUMP_BASE.
//
#define VIRTUAL_LUMP_BASE 0x8000
#define MAX_VIRTUAL_LUMPS 8
...
static vlump_t vlumps[MAX_VIRTUAL_LUMPS];
static int nvlumps = 0;

void W_AddVirtualLump(const char *name, const void *data, int size)
{
    if (nvlumps >= MAX_VIRTUAL_LUMPS)
        I_Error("W_AddVirtualLump: table full");
    memset(vlumps[nvlumps].name, 0, 8);
    strncpy(vlumps[nvlumps].name, name, 8);
    vlumps[nvlumps].data = data;
    vlumps[nvlumps].size = size;
    nvlumps++;
}
...
int PUREFUNC W_CheckNumForName(const char *name)
{
    int_64_t nameint = 0;
    strncpy((char*)&nameint, name, 8);
    for (int i = 0; i < nvlumps; i++)
    {
        if (*(int_64_t*)vlumps[i].name == nameint)
            return VIRTUAL_LUMP_BASE + i;
    }
...
{
    if (lump >= VIRTUAL_LUMP_BASE)
    {
        int vi = lump - VIRTUAL_LUMP_BASE;
        if (vi < nvlumps) return vlumps[vi].name;
        return NULL;
    }
...
int PUREFUNC W_LumpLength(int lump)
{
    ...
    {
        int vi = lump - VIRTUAL_LUMP_BASE;
        if (vi < nvlumps) return vlumps[vi].size;
        I_Error("W_LumpLength: invalid virtual lump %i", lump);
        return 0;
    }
...
const void* PUREFUNC W_CacheLumpNum(int lump)
{
    ...
    {
        int vi = lump - VIRTUAL_LUMP_BASE;
        if (vi < nvlumps) return vlumps[vi].data;
        return NULL;
    }
