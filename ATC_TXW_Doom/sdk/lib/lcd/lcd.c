...
void lcd_module_run(uint16_t *w,uint16_t *h,uint8_t *rotate){
	...
	if(PIN_LCD_RESET1 != 255){
		gpio_iomap_output(PIN_LCD_RESET1,GPIO_IOMAP_OUTPUT); 
		gpio_set_val(PIN_LCD_RESET1,0);
		os_sleep_ms(100);
		gpio_set_val(PIN_LCD_RESET1,1);
		os_sleep_ms(100);
	}
